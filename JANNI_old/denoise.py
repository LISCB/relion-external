...
def get_input_micrograph_paths(starfile_location):
    metadata = relion.read_micrographs_star(starfile_location)
    micrographs = metadata['micrographs']
    return [m.rlnMicrographName for m in micrographs]
...
def write_relion_star_final(relion_job_dir, input_star_file):
    input_micrograph_metadata = relion.read_micrographs_star(input_star_file)
...
def run_as_master(args):
    ...
    print('JANNI Version: 0.1.2', flush=True)
