...
if [ ! -z "${TOPAZ_PYTHON}" ]; then
    PYTHON=${TOPAZ_PYTHON}
elif [ ! -z "${TOPAZ_GPU_PYTHON}" ]; then
    PYTHON=${TOPAZ_GPU_PYTHON}
elif [ ! -z "${TOPAZ_CPU_PYTHON}" ]; then
    PYTHON=${TOPAZ_CPU_PYTHON}
else
    PYTHON='python'
fi
...
PYTHONPATH=$SCRIPT_DIR:$PYTHONPATH $PYTHON $SCRIPT_DIR/Topaz/pick_train.py "$@"
